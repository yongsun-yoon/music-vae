# Copied from https://github.com/magenta/magenta/blob/main/magenta/models/music_vae/trained_model.py
...
class TrainedModel(object):
    ...
    def __init__(self, config, batch_size, checkpoint_dir_or_path=None,
        ...
        with tf.Graph().as_default():
            ...
            if self._config.hparams.z_size:
                q_z = model.encode(
                    self._inputs, self._inputs_length, self._controls)
                self._mu = q_z.loc
            ...
            if var_name_substitutions is not None:
                ...
                for v in tf.global_variables():
                    ...
                    if var_name != v.name[:-2]:
                        tf.logging.info('Renaming `%s` to `%s`.',
                                        v.name[:-2], var_name)
                    var_map[var_name] = v
    ...
    def encode(self, note_sequences, assert_same_length=False):
        ...
        for note_sequence in note_sequences:
            extracted_tensors = self._config.data_converter.to_tensors(
                note_sequence)
    ...
    def decode_to_tensors(self, z, length=None, temperature=1.0, c_input=None,
        ...
        for i in range(len(z) // batch_size):
            ...
            if return_full_results:
                outputs.extend(self._sess.run(
                    self._decoder_results, feed_dict))
            else:
    ...
    def interpolate(self, start_sequence, end_sequence, num_steps,
        ...
        _, mu, _ = self.encode(
            [start_sequence, end_sequence], assert_same_length)
